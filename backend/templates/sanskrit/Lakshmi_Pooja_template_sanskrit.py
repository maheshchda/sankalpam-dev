"""
Sanskrit Sankalpam Template Generator for Lakshmi Pooja (Devanagari).
Uses Divine API with lan=hi for panchang; static text in Sanskrit/Devanagari.
"""
...
async def generate_sankalpam_text(data: Dict) -> str:
    """Generate Sanskrit (Devanagari) sankalpam text for Lakshmi Pooja."""
    lines = []

    lines.append("ॐ श्रीमहालक्ष्म्यै नमः ॥")
    lines.append("")
    ...
    pooja_display = "श्री लक्ष्मी पूजा" if (pooja_name or "").strip().lower() == "lakshmi pooja" else (pooja_name or "")
    lines.append(f"{pooja_display} पूजनं करिष्ये ।")
    lines.append("")
...
async def _get_panchang_from_divine_api(data: Dict) -> Dict[str, str]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            result["rithou"] = _SA_RITU.get(now.month, "")
            result["vAsara"] = _SA_WEEKDAYS.get(now.strftime("%A").lower(), now.strftime("%A"))
            result["ayanam"] = "उत्तरायणे" if 1 <= now.month <= 6 else "दक्षिणायणे"

    except Exception as e:
        print(f"[Lakshmi Sanskrit Template] Error fetching from Divine API: {e}")
        return _get_fallback_panchang(data)
...
def _get_fallback_panchang(data: Dict) -> Dict[str, str]:
    ...
    return {
        "rithou": _SA_RITU.get(now.month, ""),
        "mAsE": now.strftime("%B"),
        "pakshE": fallback.get("paksha", ""),
        "thithou": fallback.get("tithi", ""),
        "vAsara": _SA_WEEKDAYS.get(now.strftime("%A").lower(), now.strftime("%A")),
