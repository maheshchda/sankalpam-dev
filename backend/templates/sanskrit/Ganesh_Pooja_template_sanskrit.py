"""
Sanskrit Sankalpam Template Generator for Ganesh Pooja (Devanagari).
...
async def generate_sankalpam_text(data: Dict) -> str:
    """Generate Sanskrit (Devanagari) sankalpam text for Ganesh Pooja."""
    lines = []
...
async def _get_panchang_from_divine_api(data: Dict) -> Dict[str, str]:
    ...
    try:
        ...
        print(f"[Ganesh Sanskrit Template] Error fetching from Divine API: {e}")
        return _get_fallback_panchang(data)
...
async def _get_geographical_reference(data: Dict) -> str:
    """Geographical reference in Devanagari (continent-wise dvīpa / varṣa)."""
