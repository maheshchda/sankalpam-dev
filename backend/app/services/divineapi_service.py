...
{{sankalpa_family_elaboration_te}}
...
def _get_sankalpam_template_path(language_code: str, pooja_name: Optional[str] = None, prefer_python: bool = True) -> Optional[Path]:
    ...
    For Telugu + Ganesh/Ganesha Pooja: use Ganesh_Pooja_template_telugu.py / .txt only.
    For other poojas use pooja-specific or generic.
...
_TE_GEO_FALLBACKS = {
    ...
    "ganga nadi tere": "గంగా నదీ తటే",
}


def _ascii_fold_lower(s: str) -> str:
    """ASCII-only lowercase fold for substring checks (helps when APIs use homoglyphs in 'energy', etc.)."""
    return unicodedata.normalize("NFKD", s).encode("ascii", "ignore").decode("ascii").lower()


def _is_brand_or_utility_geo_name(s: str) -> bool:
    """True if this names a utility/brand, not a natural feature (e.g. Green Mountain Energy)."""
    if not s or not str(s).strip():
        return False
    t = unicodedata.normalize("NFKC", str(s)).strip()
    low = t.lower()
    fold = _ascii_fold_lower(t)
    if "energy" in low or "energy" in fold or "green mountain power" in low:
        return True
    if re.search(r"(?i)green\s+mountain", t) and (
        "energy" in low or "power" in low or "electric" in low
        or "energy" in fold or "power" in fold or "electric" in fold
    ):
        return True
    return False


def _telugu_geographical_name_fragment(name: str) -> str:
    """English place/feature names → Telugu script for sankalpam (no mixed Latin in output)."""
    if not name or not str(name).strip():
        return ""
    n = str(name).strip()
    if any("\u0c00" <= c <= "\u0c7f" for c in n):
        return n
    return _latin_name_to_telugu(n)


def _normalize_primary_geographical_feature_to_telugu(primary: str) -> str:
    """
    Convert primary_geographical_feature from geo APIs to Telugu-only phrasing.
    Legacy format: 'Mount Foo parvata pArshvE' (English + IAST) → Telugu + పర్వత పార్శ్వే
    """
    if not primary or not str(primary).strip():
        return ""
    s = unicodedata.normalize("NFKC", str(primary).strip())
    low = s.lower()
    if low in _TE_GEO_FALLBACKS:
        return _TE_GEO_FALLBACKS[low]
    # Block before transliteration — otherwise "Green Mountain Energy" becomes Telugu script of the brand name.
    if _is_brand_or_utility_geo_name(s):
        return _TE_GEO_FALLBACKS["sacred tirtha"]
    # Older location_service: "Name parvata pArshvE"
    m = re.match(r"^(.+?)\s+parvata\s+p[aA]rshv[eE]?\s*$", s, re.IGNORECASE)
    if m:
        inner = m.group(1).strip()
        if _is_brand_or_utility_geo_name(inner):
            return _TE_GEO_FALLBACKS["sacred tirtha"]
        name_te = _telugu_geographical_name_fragment(inner)
        return f"{name_te} పర్వత పార్శ్వే" if name_te else _TE_GEO_FALLBACKS["sacred tirtha"]
    # Plain English mountain/POI name (no Telugu letters)
    if any("a" <= c.lower() <= "z" for c in s) and not any("\u0c00" <= c <= "\u0c7f" for c in s):
        name_te = _telugu_geographical_name_fragment(s)
        return f"{name_te} పర్వత పార్శ్వే" if name_te else _TE_GEO_FALLBACKS["sacred tirtha"]
    return s


def sanitize_telugu_geo_feature_line(s: str) -> str:
    """
    Final guard for sankalpam geography lines: block utility brands and any Latin/IAST
    that slipped through (templates must never print English company names).
    """
    if not s or not str(s).strip():
        return s or ""
    t = unicodedata.normalize("NFKC", str(s)).strip()
    if _is_brand_or_utility_geo_name(t):
        return _TE_GEO_FALLBACKS["sacred tirtha"]
    # Strip legacy Latin suffix if still present
    if re.search(r"parvata\s+p[aA]rshv", t, re.I):
        t = re.sub(r"(?i)\s*parvata\s+p[aA]rshv[eE]?\s*", " ", t).strip()
        if _is_brand_or_utility_geo_name(t):
            return _TE_GEO_FALLBACKS["sacred tirtha"]
        if re.search(r"[a-zA-Z]", t):
            return f"{_telugu_geographical_name_fragment(t)} పర్వత పార్శ్వే"
        return _TE_GEO_FALLBACKS["sacred tirtha"]
    return t
...
def _telugu_geographical_feature_from_data(data: dict) -> str:
    ...
    if primary:
        low = primary.lower()
        if low in _TE_GEO_FALLBACKS:
            return sanitize_telugu_geo_feature_line(_TE_GEO_FALLBACKS[low])
        return sanitize_telugu_geo_feature_line(_normalize_primary_geographical_feature_to_telugu(primary))
    ...
    if ocean and str(ocean).strip():
        return sanitize_telugu_geo_feature_line(
            f"{_telugu_geographical_name_fragment(str(ocean).strip())} సముద్ర తీరే"
        )
    if sea and str(sea).strip():
        return sanitize_telugu_geo_feature_line(
            f"{_telugu_geographical_name_fragment(str(sea).strip())} సముద్ర తీరే"
        )
    if river and str(river).strip():
        return sanitize_telugu_geo_feature_line(
            f"{_telugu_geographical_name_fragment(str(river).strip())} నదీ తటే"
        )
    if mountain and str(mountain).strip():
        mn = str(mountain).strip()
        if _is_brand_or_utility_geo_name(mn):
            return sanitize_telugu_geo_feature_line(_TE_GEO_FALLBACKS["sacred tirtha"])
        return sanitize_telugu_geo_feature_line(f"{_telugu_geographical_name_fragment(mn)} పర్వత పార్శ్వే")
    return sanitize_telugu_geo_feature_line(
        _telugu_geographical_name_fragment(data.get("nearby_river") or "గంగా") + " నదీ తటే"
    )
...
async def _generate_telugu_sankalpam(data: dict, pooja_name: Optional[str] = None) -> str:
    ...
    if python_result:
        data["_sankalpa_highlight_names"] = []
    ...
    try:
        from app.services.sankalpa_family_builder import build_telugu_sankalpa_extras

        extras, highlight_names = build_telugu_sankalpa_extras(
            user_name_te=data.get("user_name") or "",
            gotra_te=data.get("gotram") or "",
            birth_nak_te=birth_nak_te,
            birth_rashi_te=birth_rashi_te,
            family_members_payload=data.get("family_members") or [],
            intent_key=data.get("sankalpa_intent"),
        )
        data.update(extras)
        data["_sankalpa_highlight_names"] = highlight_names
    except Exception as e:
        print(f"[Sankalpa] Telugu family elaboration skipped: {e}")
        data.setdefault("sankalpa_family_elaboration_te", "")
        data.setdefault("family_list_te", "")
        data.setdefault("gotra_suffix", "")
        data.setdefault("gotra_gotrodabhavasya", "")
        data.setdefault("sankalpa_intent_phrase_te", "")
        data["_sankalpa_highlight_names"] = []
    ...
    replacements = {
        ...
        "{{pooja_name}}": pooja_te,
        "{{sankalpa_family_elaboration_te}}": data.get("sankalpa_family_elaboration_te", ""),
        "{{family_list_te}}": data.get("family_list_te", ""),
        "{{gotra_suffix}}": data.get("gotra_suffix", ""),
        "{{gotra_gotrodabhavasya}}": data.get("gotra_gotrodabhavasya", ""),
        "{{sankalpa_intent_phrase_te}}": data.get("sankalpa_intent_phrase_te", ""),
        "{{karta_line_te}}": data.get("karta_line_te", ""),
    }
    out = template
...
async def generate_sankalpam(
    ...
    Returns (text, highlight_names) where highlight_names are Telugu name segments for UI bolding.
    """

    def _with_highlights(text: str) -> Tuple[str, List[str]]:
        return text, list(sankalpam_data.pop("_sankalpa_highlight_names", []))
    ...
    sankalpam_data = {
        ...
        "language": language,
        "sankalpa_intent": sankalpa_intent,
        "family_members": [
            {
                "name": member.name,
                "relation": member.relation,
                "relation_raw": member.relation,
                "gotram": user.gotram,  # Usually same gotram
            }
            for member in family_members
        ],
    }
    if override_gotram and str(override_gotram).strip():
        sankalpam_data["gotram"] = str(override_gotram).strip()
    if override_birth_nakshatra and str(override_birth_nakshatra).strip():
        sankalpam_data["birth_nakshatra"] = str(override_birth_nakshatra).strip()
    ...
    if effective_code == "te":
        ...
        for m in sankalpam_data["family_members"]:
            ...
            rel_raw = (m.get("relation_raw") or m.get("relation") or "").strip()
            m["relation"] = _english_to_telugu(rel_raw, _TE_RELATION) if rel_raw else ""
    ...
    if force_telugu:
        ...
        out = await _ensure_telugu_for_user(telugu_text, sankalpam_data, pooja_name, "te")
        return _with_highlights(out)
    ...
    if effective_code in ("hi", "sa"):
        ...
        if python_text and python_text.strip():
            out = await _ensure_telugu_for_user(python_text, sankalpam_data, pooja_name, effective_code)
            return _with_highlights(out)
    ...
    if settings.divineapi_key:
        try:
            async with httpx.AsyncClient() as client:
                ...
                api_payload = {k: v for k, v in sankalpam_data.items() if not str(k).startswith("_")}
                response = await client.post(url, json=api_payload, headers=headers, timeout=30.0)
                
                if response.status_code == 200:
                    data = response.json()
                    api_text = data.get("sankalpam_text") or ""
                    if api_text:
                        out = await _ensure_telugu_for_user(api_text, sankalpam_data, pooja_name, effective_code)
                        return _with_highlights(out)
                    result = await generate_standard_sankalpam(sankalpam_data)
                    out = await _ensure_telugu_for_user(result, sankalpam_data, pooja_name, effective_code)
                    return _with_highlights(out)
                else:
